...
" deno
"let g:denops#deno = "deno --unstable"
...
function! SilentMake()
  ...
  if v:shell_error != 0
    ...
    " クイックフィックスリスト内の最初のエラーにジャンプ
    let l:entry = 1
    while l:entry <= len(getqflist())
      if getqflist()[l:entry - 1]['valid'] && getqflist()[l:entry - 1]['type'] == 'E'
        exec 'cc' l:entry
        break
      endif
      let l:entry += 1
    ...
    echohl Search | echo "✅ Build succeeded!" | echohl None
  endif
endfunction

command! -nargs=* Make call SilentMake()
